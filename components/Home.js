...
export default function Home({navigation}){
  ...
  const handleProjectPress = (index) => {
    navigation.navigate('Project Details', index)
  }
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        {projects.map((project, index) => {
            return (
                <ProjectView title={project.title} key={index} index={index} handleDelete={() => handleDelete(index)} handleProjectPress={() => handleProjectPress(index)}/>
            )
        }
        )}
      </ScrollView>
